...
def _build(engine, stt_config, sample_rate):
    if engine == "whisper":
...
def create_recognizer(stt_config, sample_rate=16000):
    """
    ...
    try:
        recognizer = _build(engine, stt_config, sample_rate)
        recognizer.load()
        ...
        recognizer = _build(fallback, stt_config, sample_rate)
        recognizer.load()
        return recognizer
